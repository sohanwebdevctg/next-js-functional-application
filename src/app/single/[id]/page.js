...
const SingleProductPage = ({ params, searchParams }) => {
  ...
  useEffect(() => {
    if (id && name) {
      const matchItem = furnitureData.find((category) =>
        category.items.some(
          (item) => item.id === parseInt(id) && item.name === name
        )
      );

      if (matchItem) {
        const item = matchItem.items.find(
          (item) => item.id === parseInt(id) && item.name === name
        );
        setItemData(item);
  ...
  const addData = () => {
    ...
    //get previous cartData
    const cartData = localStorage.getItem('cartData');

    //check cartData
    const cartArray = cartData ? JSON.parse(cartData) : [];

    //adding new cartItem in cartArray
    cartArray.push(cartItem)

    //set the update array data localStorage
              ...
              <ul className="space-y-2 sm:space-y-4 md:space-y-3 lg:space-y-3 xl:space-y-4">
                ...
                <li className="flex items-center gap-6 sm:gap-3 md:gap-3 lg:gap-4 xl:gp-6">
                  ...
                  <p className="flex gap-1">
                    {itemData?.colorItem?.map((data, index) => (
                      <span
                        key={index}
